		...
		await this.consumer.run({
			eachMessage: async ({ topic, partition, message }) => {
				...
				try {
					...
					this.monitoring.observeHistogram('message_processing_time', Date.now() - startTime);
				} catch (error) {
					this.monitoring.incrementCounter('message_processing_errors');
					this.logger.error(`Error processing message from topic ${topic}, partition ${partition}`, error);
