...
export class ClickProcessorService implements OnModuleDestroy {
	...
	private deadLetterQueue: Map<string, { event: any; failures: number }> = new Map();
	...
	private async flushAllBuffers() {
		await Promise.allSettled([this.flushClickBuffer(), this.flushAnalyticsBuffer(), this.processDeadLetterQueue()]);
	}
	...
	private async flushClickBuffer() {
		...
		try {
			...
			this.monitoring.observeHistogram('click_processing_duration', Date.now() - startTime);
		} catch (error) {
	...
	private async flushAnalyticsBuffer() {
		...
		try {
			...
			this.monitoring.observeHistogram('analytics_processing_duration', Date.now() - startTime);
		} catch (error) {
	...
	private async processDeadLetterQueue() {
		...
		if (retryItems.length > 0) {
			const clickEvents = retryItems.filter(([key]) => key.startsWith('click:')).map(([, value]) => value.event);
				...
				try {
					...
					clickEvents.forEach((event) => {
						...
						this.deadLetterQueue.set(key, {
							event,
							failures: (retryItems.find(([k]) => k === key)?.[1].failures || 0) + 1,
						});
					});
				}
			}
	...
	private async saveAnalyticsBatch(batch: AnalyticsEvent[], retries = this.MAX_RETRIES) {
		const client = this.dbService.getClient();
      ...
      `,
        ...
        `,
				params: [events.length, deviceCounts.mobile, deviceCounts.tablet, deviceCounts.desktop, shortCode, day],
			});
		});
		...
		try {
			...
			if (retries > 0) {
				this.logger.warn(`Retrying analytics batch save, attempts left: ${retries - 1}`);
